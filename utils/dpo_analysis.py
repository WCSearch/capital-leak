"""
DPO Time-State Analysis Module

Transforms DPO from a single metric into a diagnostic tool that identifies:
1. WHERE ap_invoices time is accumulating (time states)
...
Parallel structure to DSO analysis but for ap_invoices management.
"""
...
# Time-state definitions with metadata (DPO-specific)
TIME_STATE_CONFIG = {
    'received_not_valued': {
    ...
    'obsolete_unwritten': {
        ...
        'priority': 'MEDIUM',
        'color': 'amber',
        'fix_action': 'Write off obsolete ap_invoices',
        'timeline': '15 days',
        'order': 4
    },
    'slow_moving': {
...
def classify_dpo_time_state(transaction: Dict, event_logs: List[Dict]) -> str:
    """
    Classify a DPO transaction into one of 6 mutually exclusive time states.

    Args:
        transaction: Inventory record
        event_logs: List of event log records for this invoice
    ...
    # Check for obsolete ap_invoices
    if days_outstanding > 180 and amount > 10000:
        return 'obsolete_unwritten'

    # Check for slow-moving ap_invoices
    if days_outstanding > 90:
        return 'slow_moving'

    # Check for excess ap_invoices
    if days_outstanding > 60:
        return 'excess_ap_invoices'

    # Default: slow-moving to ensure visibility
    return 'slow_moving'


def analyze_dpo_time_states(
    ap_invoices_df: pd.DataFrame,
    movements_df: pd.DataFrame,
    event_logs_df: pd.DataFrame
) -> Dict:
    """
    Analyze all DPO ap_invoices and group by time state.

    Args:
        ap_invoices_df: DataFrame of ap_invoices transactions
        movements_df: DataFrame of ap_invoices movements (optional)
    ...
    ap_invoices = ap_invoices_df.to_dict('records')
    ...
    # Classify each ap_invoices item
    for item in ap_invoices:
        time_state = classify_dpo_time_state(item, event_logs)
...
def get_dpo_time_state_summary(time_state_groups: Dict) -> List[Dict]:
    """
    Get a summary of DPO time states for dashboard display.
...
def calculate_recovery_confidence(transaction: Dict, time_state: str) -> Dict:
    """
    Calculate recovery confidence level for an ap_invoices item.
    ...
    if time_state == 'excess_ap_invoices':
        return {
...
def analyze_dpo_recovery_confidence(time_state_groups: Dict) -> Dict:
    """
    Analyze recovery confidence across all DPO time states.
    ...
    # Calculate confidence for each ap_invoices item in each time state
    for state_key, group in time_state_groups.items():
        if state_key == 'normal':
            continue
...
def analyze_dpo_root_causes(
    ap_invoices_df: pd.DataFrame,
    movements_df: pd.DataFrame,
    event_logs_df: pd.DataFrame
) -> List[Dict]:
    """
    Analyze ap_invoices and group by root cause.

    If event_logs_df has data:
        → Use event_log_analyzer_dpo.synthesize_dpo_root_causes()
    ...
    if has_event_logs:
        ...
        from utils.event_log_analyzer_dpo import synthesize_dpo_root_causes

        # Get detailed root cause classification for each item
        classified_df = synthesize_dpo_root_causes(
            ap_invoices_df,
        ...
        classified_df = classify_dpo_root_causes(
            ap_invoices_df,
            movements_df if movements_df is not None else pd.DataFrame()
        )
...
def generate_dpo_execution_queue(root_cause_list: List[Dict]) -> List[Dict]:
    """
    Generate DPO execution queue ranked by efficiency.
